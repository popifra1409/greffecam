<div class="flex items-center gap-3 mr-4">
    ...
    {{-- Nouveau Dossier --}}
    <a 
        href="{{ route('filament.decision-recours.resources.dossiers.create') }}" 
    ...
    {{-- Nouvelle Décision --}}
    <a 
        href="{{ route('filament.decision-recours.resources.decisions.create') }}" 
        class="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/20 transition"
    ...
    @if($recoursEnAttente > 0)
        ...
        >
            ...
            <span class="inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold text-white bg-orange-600 rounded-full">
                {{ $recoursEnAttente }}
            </span>
        </a>
    @endif
    ...
    {{-- Compteur Alertes --}}
    @php
        $alertesNonLues = \App\Models\AlerteRecours::where('est_lue', false)->count();
    @endphp
    
    @if($alertesNonLues > 0)
        <a 
            href="/decision-recours" 
            class="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition hidden lg:flex"
            title="Alertes de délais de recours"
        >
            <x-heroicon-o-bell-alert class="w-5 h-5 animate-pulse" />
            <span class="hidden xl:inline">Alertes</span>
